...
To use and install the cli tool that uses the library with `cargo`, clone the repo and run `cargo install --path .` in the project directory. If cargo was install using rustup that usually means the resulting binary will be in your PATH as `aoc-cli`. Uninstall with `cargo uninstall aoc-cli`. 
